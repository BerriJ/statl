...
.font130[
.content-box-blue[
`r fontawesome::fa("chalkboard-teacher", fill = "black")` &thinsp; Introduction
`r fontawesome::fa("table", fill = "black")` &thinsp; Data and Variables
`r fontawesome::fa("search", fill = "black")` &thinsp; Validation Approach
`r fontawesome::fa("terminal", fill = "black")` &thinsp; Analysis
`r fontawesome::fa("hammer", fill = "black")` &thinsp; Robustness
`r fontawesome::fa("flag-checkered", fill = "black")` &thinsp; Final Model
`r fontawesome::fa("comments", fill = "black")` &thinsp; Discussion
]]]
...
- Variables: name, country, price, amount sold per week, market share, segments, review-related variables, etc.
- Density of litre is nearly the same for train (from 0 to 184200) and test (from 0 to 184398)
...
- Further robustness checks for assessing validity of results obtained
...
So clearly some predictors are useful. 
...
class: sydney-green, center, middle
...
- Same variables were chosen in every fold
- RMSE: $6495$
...
???
- 10 terminal nodes (minimum of 19 observaions (default in function))
...
- Cost complexity pruning: Penalizing larger trees, i.e., trees with more terminal nodes
- RMSE: $7936$
...
???
- Single regression trees are likely to overfit data; thus bad test set performance
- Smaller tree may lead to lower variance, better interpretation at cost of some bias
- Goal: Selecting Subtree leading to lowest test error
- Cost complexity pruning: Minimizing SSR with penalty (alpha*No. of terminal nodes)
- Tuning parameter is chosen via CV
- Easier to interpret and more obs in terminal nodes
- RMSE with 7936 is much higher than RMSE for full tree with 6495
...
- Averaging of results afterward
...
???
- Decision trees suffer from potentially high variance
- Averaging a set of observations reduces variance
- Sample size: 66.6%
- Already small number of bootstrap samples (like 15) outperform single tree in terms of RMSE
...
???
- RMSE does slightly depend on fold but common tendencies can be seen
- Fast decrease of RMSE when starting to average
- Flattens out from 25 trees upwards (increases slightly towards 100)

- RMSE for Bagging, averaged over 5 trees: 4578
- RMSE for Bagging, averaged over 25 trees: 4306 (Lowest in our research)

- Best method up to then: Splines with 5507
---
...
- Tuning parameters: Number of trees grown and number of randomly chosen variables considered at each split
...
???
- Different tree at each iteration of random forest
- Thus reduction in variance compared to bagging
- RMSE for range of values: Up to 25 Trees and up to 100 variables
- Model with 20 trees and 80 variables still has RMSE of 4456 (increase less than 1.9%)
...
???
- Bagging (RMSE 4305) outperforms random forest model (4374) but rf only takes about 15% of time bagging needs
...
??? 
- Mean Increase in RMSE when omitting the variable shown
- Top 20 variables: many price variables in top 20
- mean market share in color segment
- price
- lp: log price
- pri_m: mean real litre price (base jan 2004)
- rprice_litre: real litre price
- if wine is red
- price_segml: Price segment of wine: low
...
???
- marginal effects of variables on litres sold per week
- nonlinear dependecies (thats why linear methods did not perform that great)
- Mention the dummys
- Validation of intuitive assumptions
  - Higher marketshare $\rightarrow$ more litres sold
  - High prices $\rightarrow$ less litres sold
  - segm_red has positive influence (also in single tree)
...
- Sequential growing of trees using the residuals of previous grown trees
...
- Best boosting model: $25$ trees, interaction depth of $15$, $\lambda =0.5$
- RMSE: $4479$
]
]

???
- rather small trees are used to improve boosting tree slowly
- Taking residuals from current tree, fitting small tree to these and fitting fitted tree into current boosting tree where performance could be improved
- Expansion of tree but also growing dependence
- Other tuning parameter in R, in gbm()-function: bag.fraction 
  - We set this to 1 so that every time the complete training set has been used
  - This outperformed the default (=0.5) by 1.9% decrease of RMSE
- Shrinkage: learning rate normally between 0.001 and 0.1 (default: 0.1) $\rightarrow$ smaller rate typically requires more trees
...
???
- Most important variables (decreasing order): ms_segm, segmred, period, price, week, lp
...
Calculation of various alternative estimations, notably:
...
- Reducing share of NAs for removing variables from the dataset
  - Beforehand: $50\%$
  - Reducing it to $20\%$
  - Loss of variables and gain of observations does not lead to higher precision

]

???
- six variables are excluded when reducing max share of NAs to 20%
- Gain of obs: from 41416 to 144994 (ca. plus 100.000)
---
...
- Performance on test data set: $6738$
  - $6546$ if we omit one outlier
...
???
- difficult comparison of train and test data set
  - Levels are missing and time series setting
  
  - mention that repo is accesible for reproduction and transparency
