...
Bootsrap aggregation (Bagging) is a special version of a random forest in which, at each split, all possible variables are taken in consideration for selection. In contrast to random forests this may lead to very similar trees at each iteration. Thus, the trees ca be highly correlated. Furthermore, bagging is computationally more demanding since all variables are taken into consideration at each split.
