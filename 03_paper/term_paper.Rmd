...
```{r, echo = FALSE}
knitr::include_graphics("../00_data/output_paper/06_lasso_vars.pdf")
```

## PCR and PLS

While lasso is a technique for variable selection, PCR and PLS are methods that reduce the dimension of the feature set. Thus, the least squares estimation is performed using a transformation of the explanatory variables.

Figure XY shows the first two principal components from a principal components analysis. The first principal components represent roughly 3\% of the total variance. This is a sign that the PCA doesn't work well in our data which is due to the large amount of dummy variables. 

```{r, echo = FALSE}
knitr::include_graphics("../00_data/output_paper/07_pca.png")
```

<!---
In principle, PLS should perform better because it takes the variance in the response variable into account when computing the new feature set.
-->
...
As regression trees may tend to grow quite large, a possible way to get smaller trees is pruning back the grown larger tree. The goal of pruning is `to select a subtree that leads to the lowest test error rate [which] we can estimate [...] using cross-validation or the validation set approach' [@James2014, p. 308].
