...
Figure XY depicts the relation between lambda and the coefficients. The crossvalidated log lambda, which minimizes the test error is $\approx `r df[2]`$. At that level a total of $\approx `r df[4]`$ out of $713$ coefficients are nonzero. The other coefficients are exactly zero due to the sparsitiy property of the lasso estimator. Furthermore, the plot includes abbreviated variable names of the 10 biggest coefficients. All of those coefficients are dummys for specific wine names.
The average RMSE of the lasso approach is $\approx `r df[1]`$. This is a slight improvement above the linear baseline model.
The lasso does not significantly reduce the feature set. It shows, that the name variable is the most important explaining the sold litre per week, followed by the Vintage, the Region, the taste segment and the Country of the wine. The least influence important variables are the review variables which are not present in the 400 biggest coefficients. 
