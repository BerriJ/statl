...
they represent interactions terms. Incluing them yields a mean \ac{rmse}
of \(\approx4862.917\) and therefore doesn't improve our model.

Another curcial decision was to exlude all variables with more than
...
\(144.994\) which is nearly the whole dataset. Estimating the random
forest model yields a \ac{rmse} of \(\approx5010.796\), which
underperformes compared to the model developed in
\autoref{chap:rand_for}. This shows, that the lost explanatory power by
exluding those 6 variables is not compensated by a higher precision due
to the gain of observations.
...
substantially less computation time.
