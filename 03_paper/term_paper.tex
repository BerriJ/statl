...
Figure \ref{fig:rfimp} shows partial dependence plots for the 10 most
important variables. The plots present the marginal effect of the
variable on the litre. Those plots clearly reveal some nonlinear
dependences in the data which explains the poor performes of the linear
models. Furthermore the figure validates some intuitive assumptions. For
example, a higher market shares leads to more litres sold while high
real prices lead to fewer litres sold. Furthermore the influence of
\(v10\_dim\) which is the \enquote{Dagens Industri} review is especially
big, when the review is very good. Figure \ref{fig:rfpardep_other}
presents the partial dependence plots for the next 10 important
variables.
...
underprerformance compared to the model developed in
\autoref{chap:rand_for}. This shows, that the loss of explanatory power
by excluding those \(6\) variables is not compensated by a higher
precision due to the gain of observations.
...
\hypertarget{partial-dependence-plots-for-random-forest}{%
\subsection{Partial Dependence Plots for Random
Forest}\label{partial-dependence-plots-for-random-forest}}
...
\hypertarget{partial-dependence-plots-for-bootstrap-aggregation}{%
\subsection{Partial Dependence Plots for Bootstrap
Aggregation}\label{partial-dependence-plots-for-bootstrap-aggregation}}

\begin{figure}
\centering
\includegraphics{../00_data/output_paper/16_pdp_boosting_main.pdf}
\caption{\label{fig:bgpardep_main}Random Forest: Partial Dependence
Plots.}
\end{figure}

\begin{figure}
\centering
\includegraphics{../00_data/output_paper/16_pdp_boosting_other.pdf}
\caption{\label{fig:bgpardep_other}Random Forest: Partial Dependence
Plots.}
\end{figure}
