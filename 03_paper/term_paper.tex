...
training set have been used. This also made sure that running the code
more than once resulted in the same fit, which isn't necessaary the case
when setting the argument to a value \(< 1\).
...
cross-validation). All obtained \ac{rmse}s can be found in table ADD
LABEL For comparison we set the \ac{rmse}s of the mean regression and
the simple linear regression as two baseline. The \ac{rmse} obtained
from the mean regression is \(\approx \ensuremath{1.334\times 10^{4}}\)
