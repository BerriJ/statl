...
\ac{pcr}{[}PCR{]}\{principal components regression\} and
\ac{pls}{[}PLS{]}\{partial least squares\} are methods that reduce the
dimension of the feature set. Thus, the least squares estimation is
...
is a sign that the \ac{pca} does not work well on our data which might
...
\ac{rmse} obtained from the mean regression is \(\approx 13340\) litres
sold per week, which is a rather high value. This becomes clear when
comparing it to the mean \ac{rmse} of the linear regression model which
is \(\approx 5572\). This shows that the linear regression gives in
principle a better forecast than the mean regression, which probably is
due to the many more variables included in the linear regression.
