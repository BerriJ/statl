...
table with summary statistics of the data set can be found in appendix
...
\autocite[][p. 303]{Hastie2013}. Simple tree-based methods use single
...
\autocite[Cf.][p. 349.]{Hastie2013}. After the data is split this
...
\caption[Bagging: RMSE's at Different Tree Sizes]{\label{fig:rmsebag}Bagging: RMSEs at Different Tree Sizes (Smoothed).}\label{fig:bag}
\end{figure}
...
overfitting but it adds variance to the in sample predictions
...
tree as well as the pruned tree, as those utilzed exactly those
variables.
