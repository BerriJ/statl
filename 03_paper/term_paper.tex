...
The 25 Tree \ac{bagging} Model (\ac{rmse} of \(\approx 4305\))
...
In this paper we tried to find an accurate forecasting model based on
the friberg gringvist wine data set. To achieve this goal we used
different techniques from the field of statistical learning methods and
compare those in terms of their mean \ac{rmse} (averaged over all folds
of the cross-validation). For comparison we set the \ac{rmse}s of the
mean regression and the simple linear regression as two baseline. The
\ac{rmse} obtained from the mean regression is
\(\approx \ensuremath{1.334\times 10^{4}}\) litres sold per week, which
is a rather high value. This becomes clear when comparing it to the mean
\ac{rmse} of the linear regression model which is \(\approx 5572\). This
shows that the linear regression gives in principle a better forecast
than the mean regression, which probably is due to the many more
variables included in the linear regression. Following the basic
regression methods we introduced the \ac{lasso} as a combination of
linear regression and variable selection, hoping that this dimension
reduction of the feature space reduces the possible overfit and
therefore improving the forecast. As seen the mean \ac{rmse} calculated
for the lasso is \(\approx 5548\) which is a small improvement compared
to the linear regression with all variables of the feature space. As
other methods for dimension reduction of the feature set we also tried
\ac{pcr} and \ac{pls}. The \ac{pls} resulted in a mean \ac{rmse} of
\(\approx 6201\) wheres the cross-validated \ac{rmse} obtained from the
\ac{pcr} has a value of \(\approx 5546\) which is nearly exact the same
value as obtained from the \ac{lasso}.
...
\caption{\label{tab:RMSE}RMSEs of every fold for every method.}
\centering
