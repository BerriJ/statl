...
# Facet Labels
pd_df.labs <- imp_df$names[1:20]
names(pd_df.labs) <- LETTERS[1:20]
...
ggsave(filename = "00_data/output_paper/11_par_dep_random_forest_other.pdf", 
        plot = par_dep_rf , width = 7, height = 5)
...
for(i in 1:20){
  pd_list[[i]] <- plot(boosting_model_final, i = var_imp_vars[i], return.grid = T) %>% 
    as.data.frame() %>%
    round(2)
  colnames(pd_list[[i]]) <- c(LETTERS[i],"y")
  cat(i)
...
# Facet Labels
pd_df.labs <- var_imp_vars[1:20]
names(pd_df.labs) <- LETTERS[1:20]

par_dep_boost <- ggplot(pd_df, aes(y = y/1000, x = x)) + 
  geom_line(col = "#0080db", size = 1) +
  theme_minimal() +
  theme(strip.background = element_rect(fill="lightgrey", colour = "lightgrey"),
        panel.border = element_rect(linetype = "solid", fill = NA, color = "grey")) + 
  ...
  scale_x_continuous(breaks = NULL) + 
  ylab("Litre (Thousands)") + 
  xlab(label = NULL); par_dep_boost

ggsave("00_data/output_paper/16_pdp_boosting_main.pdf", par_dep_boost,
      width = 7, height = 5)
